...
UI/UX 자바스크립트 스타일 가이드는 [Airbnb 자바스크립트 스타일 가이드](https://github.com/airbnb/javascript)를 기준으로 작성되었습니다.
...
## 서문
  ...
  - 자바스크립트 개발을 위한 스타일가이드를 제시하고 있지만, 그게 규정이라는 의미는 아닙니다.
  - 제일 중요한 것은 **일관성 있는 코딩 스타일을 위한 규칙** 입니다.
  - 프로젝트에 어떤 스타일을 사용하던 간에 반드시 지켜져야 합니다.
  - 이 문서를 프로젝트의 일관성, 가독성, 유지보수 등을 위한 참고자료로 활용해주세요.
  - **자바스크립트 스타일에 대한 의견이 있으시면 언제든지 문의하십시오.**

## 가독성

  - [1.1](#1.1) <a name='1.1'></a> 문서의 기본 탭 설정은 4로 설정하십시오.
  - [1.2](#1.2) <a name='1.2'></a> 절대 스페이스와 탭을 혼용하여 사용하지 마십시오.
  - [1.3](#1.3) <a name='1.3'></a> javascript나 html 태그 작성시에 depth 표기는 탭(공백)으로 하십시오.
  - [1.4](#1.4) <a name='1.4'></a> 중괄호{}, 괄호() 줄 바꾸기
  - [1.5](#1.5) <a name='1.5'></a> **프로젝트에 일관된 탭 크기, 줄 바꿈을 사용하십시오.**
...
> [Airbnb](https://github.com/airbnb/javascript/blob/master/README.md#types)와 동일합니다.
  ...
  - [1.1](#1.1) <a name='1.1'></a> **Primitives**: primitive type(원시타입)은 그 값을 직접 조작합니다.
    + `string`
    + `number`
    + `boolean`
    + `null`
...
> [Airbnb](https://github.com/airbnb/javascript/blob/master/README.md#references)와 동일합니다.
  ...
  - [2.1](#2.1) <a name='2.1'></a> 모든 변수는 `const` 를 사용하고, `var` 를 사용하지 마십시오.
  > 왜? 변수를 재할당 할 수 없으므로, 버그로 이어지고 이해하기 어려운 코드가 되는것을 방지합니다.
  ...
  - [2.2](#2.2) <a name='2.2'></a> 변수를 재할당 해야한다면 `var` 대신 `let` 을 사용하십시오.
...
- [3.6](#3.6) <a name='3.6'></a> `-`문자와 같이 속성명으로 지정할수 없는 경우에는 꼭 싱글쿼터('')를 붙인다. eslint: [`quote-props`](http://eslint.org/docs/rules/quote-props.html)
...
## 문자열(Strings)
> `6.2` 항목을 제외하고는 [Airbnb](https://github.com/airbnb/javascript/blob/master/README.md#strings)와 동일합니다.
  ...
  - [6.4](#6.4) <a name='6.4'></a> 절대로 `eval()` 을 사용하지 않는다. 성능 및 원격 코드 실행 취약점이 발견되어 이슈가 될 수 있다.
...
`7.1 ~ 2`, `7.11 ~ 14` 항목을 제외하고는 [Airbnb](https://github.com/airbnb/javascript/blob/master/README.md#functions)와 동일합니다.
 ...
 - [7.10](#7.10) <a name='7.10'></a> 절대 새 함수를 작성하기 위해 Function constructor를 이용하지 않는다. eslint: [`no-new-func`](http://eslint.org/docs/rules/no-new-func)
    > 왜? 이런 형태로 함수를 정의하면 eval()과 비슷한 문자열로 평가되어 취약성이 올라갑니다.
   ```javascript
   // bad
...
> `8.2`, `8.4` 항목을 제외하고는 [Airbnb](https://github.com/airbnb/javascript/blob/master/README.md#arrow-functions)와 동일합니다.
  ...
  > 왜? arrow함수는 그 context의 `this` 에서 실행하는 버전의 함수를 작성합니다. 보다 간결한 구문이기 때문입니다.
  ...
  > 왜? Syntactic sugar. 여러 개의 함수가 연결된 경우 가독성이 좋아집니다.
...
> [Airbnb](https://github.com/airbnb/javascript/blob/master/README.md#classes--constructors)와 동일합니다.
  ...
  - [9.4](#9.4) <a name='9.4'></a> toString()을 작성하는것을 허용하지만 올바르게 동작하는지와 side effect 가 없는지만 확인해 주십시오.
...
- [15.5](#15.5) <a name='15.5'></a>`case`, `default` 구문에서 `let`, `const`, `function`, `class`가 사용 되는 경우에는 중괄호(`{}`)를 사용한다. eslint rules: [`no-case-declarations`](http://eslint.org/docs/rules/no-case-declarations.html).
  > 왜? block scope와 가독성의 이유로 중괄호를 사용하여 구분한다.
...
> `22.4 ~ 5`, `22.10 ~ 15` 항목을 제외하고는 [Airbnb](https://github.com/airbnb/javascript/blob/master/README.md#naming-conventions)와 동일합니다.
  ...
  - [22.4](#22.4) <a name='22.4'></a>함수명이나 속성명의 앞에 `_`가 있는 경우는 private을 의미한다.  외부에서 접근할 수 있어도 사용하면 안된다. eslint: [`no-underscore-dangle`](http://eslint.org/docs/rules/no-underscore-dangle.html)
  ...
  - [22.9](#22.9) <a name='22.9'></a>약어 및 이니셜은 항상 모두 대문자이어야 한다.
...
> `25.3 ~ 4` 항목을 제외하고는  [Airbnb](https://github.com/airbnb/javascript/blob/master/README.md#jquery)와 동일합니다.
  ...
  - [25.4](#25.4) <a name='25.4'></a> find를 사용할 떄는 object query scope을 이용한다.
  ```javascript
  // bad
  $('ul', '.sidebar').hide();
